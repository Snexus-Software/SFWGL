...
#ifndef SFGL_FREE
#define SFGL_FREE(x) free(x)
#endif

#ifndef SFGL_MALLOC
#define SFGL_MALLOC(sz)       malloc(sz)
#endif
#ifndef SFGL_CALLOC
#define SFGL_CALLOC(n,sz)     calloc(n,sz)
#endif
...
#define SFGL_TEXTURE_WRAP_S               0x2802      // GL_TEXTURE_WRAP_S
#define SFGL_TEXTURE_WRAP_T               0x2803      // GL_TEXTURE_WRAP_T
#define SFGL_TEXTURE_MAG_FILTER           0x2800      // GL_TEXTURE_MAG_FILTER
#define SFGL_TEXTURE_MIN_FILTER           0x2801      // GL_TEXTURE_MIN_FILTER
#define SFGL_TEXTURE_ANISOTROPIC_FILTER   0x3000      // Anisotropic filter (custom identifier)

#define SFGL_FILTER_NEAREST               0x2600      // GL_NEAREST
#define SFGL_FILTER_LINEAR                0x2601      // GL_LINEAR
#define SFGL_FILTER_MIP_NEAREST           0x2700      // GL_NEAREST_MIPMAP_NEAREST
#define SFGL_FILTER_NEAREST_MIP_LINEAR    0x2702      // GL_NEAREST_MIPMAP_LINEAR
#define SFGL_FILTER_LINEAR_MIP_NEAREST    0x2701      // GL_LINEAR_MIPMAP_NEAREST
#define SFGL_FILTER_MIP_LINEAR            0x2703      // GL_LINEAR_MIPMAP_LINEAR

#define SFGL_WRAP_REPEAT                  0x2901      // GL_REPEAT
#define SFGL_WRAP_CLAMP                   0x812F      // GL_CLAMP_TO_EDGE
#define SFGL_WRAP_MIRROR_REPEAT           0x8370      // GL_MIRRORED_REPEAT
#define SFGL_WRAP_MIRROR_CLAMP            0x8742      // GL_MIRROR_CLAMP_EXT

// Matrix modes (equivalent to OpenGL)
#define SFGL_MODELVIEW                    0x1700      // GL_MODELVIEW
#define SFGL_PROJECTION                   0x1701      // GL_PROJECTION
#define SFGL_TEXTURE                      0x1702      // GL_TEXTURE

// Primitive assembly draw modes
#define SFGL_LINES                        0x0001      // GL_LINES
#define SFGL_TRIANGLES                    0x0004      // GL_TRIANGLES
#define SFGL_QUADS                        0x0007      // GL_QUADS
...
typedef enum {
    FILTER_POINT = 0,               // No filter, just pixel approximation
...
//------------------------------------------------------------------------------------
    LOADERTYPE void SFGLMatrixMode(int mode);                    // Choose the current matrix to be transformed
    LOADERTYPE void SFGLPushMatrix(void);                        // Push the current matrix to stack
    LOADERTYPE void SFGLPopMatrix(void);                         // Pop lattes inserted matrix from stack
    LOADERTYPE void SFGLLoadIdentity(void);                      // Reset current matrix to identity matrix
    LOADERTYPE void SFGLTranslatef(float x, float y, float z);   // Multiply the current matrix by a translation matrix
    LOADERTYPE void SFGLRotatef(float angleDeg, float x, float y, float z);  // Multiply the current matrix by a rotation matrix
    LOADERTYPE void SFGLScalef(float x, float y, float z);       // Multiply the current matrix by a scaling matrix
    LOADERTYPE void SFGLMultMatrixf(float* matf);                // Multiply the current matrix by another matrix
    LOADERTYPE void SFGLFrustum(double left, double right, double bottom, double top, double znear, double zfar);
    LOADERTYPE void SFGLOrtho(double left, double right, double bottom, double top, double znear, double zfar);
    LOADERTYPE void SFGLViewport(int x, int y, int width, int height); // Set the view port area

    //------------------------------------------------------------------------------------
    // Functions Declaration - Vertex level operations
    //------------------------------------------------------------------------------------
    LOADERTYPE void SFGLBegin(int mode);                         // Initialize drawing mode (how to organize vertex)
    LOADERTYPE void SFGLEnd(void);                               // Finish vertex providing
    LOADERTYPE void SFGLVertex2i(int x, int y);                  // Define one vertex (position) - 2 int
    LOADERTYPE void SFGLVertex2f(float x, float y);              // Define one vertex (position) - 2 float
    LOADERTYPE void SFGLVertex3f(float x, float y, float z);     // Define one vertex (position) - 3 float
    LOADERTYPE void SFGLTexCoord2f(float x, float y);            // Define one vertex (texture coordinate) - 2 float
    LOADERTYPE void SFGLNormal3f(float x, float y, float z);     // Define one vertex (normal) - 3 float
    LOADERTYPE void SFGLColor4ub(byte r, byte g, byte b, byte a);    // Define one vertex (color) - 4 byte
    LOADERTYPE void SFGLColor3f(float x, float y, float z);          // Define one vertex (color) - 3 float
    LOADERTYPE void SFGLColor4f(float x, float y, float z, float w); // Define one vertex (color) - 4 float

    //------------------------------------------------------------------------------------
    // Functions Declaration - OpenGL equivalent functions (common to 1.1, 3.3+, ES2)
    // NOTE: This functions are used to completely abstract SFWGL code from OpenGL layer
    //------------------------------------------------------------------------------------
    LOADERTYPE void SFGLEnableTexture(unsigned int id);                  // Enable texture usage
    LOADERTYPE void SFGLDisableTexture(void);                            // Disable texture usage
    LOADERTYPE void SFGLTextureParameters(unsigned int id, int param, int value); // Set texture parameters (filter, wrap)
    LOADERTYPE void SFGLEnableRenderTexture(unsigned int id);            // Enable render texture (fbo)
    LOADERTYPE void SFGLDisableRenderTexture(void);                      // Disable render texture (fbo), return to default frame buffer
    LOADERTYPE void SFGLEnableDepthTest(void);                           // Enable depth test
    LOADERTYPE void SFGLDisableDepthTest(void);                          // Disable depth test
    LOADERTYPE void SFGLEnableBackfaceCulling(void);                     // Enable back face culling
    LOADERTYPE void SFGLDisableBackfaceCulling(void);                    // Disable back face culling
    LOADERTYPE void SFGLEnableWireMode(void);                            // Enable wire mode
    LOADERTYPE void SFGLDisableWireMode(void);                           // Disable wire mode
    LOADERTYPE void SFGLDeleteTextures(unsigned int id);                 // Delete OpenGL texture from GPU
    LOADERTYPE void SFGLDeleteRenderTextures(RenderTexture2D target);    // Delete render textures (fbo) from GPU
    LOADERTYPE void SFGLDeleteShader(unsigned int id);                   // Delete OpenGL shader program from GPU
    LOADERTYPE void SFGLDeleteVertexArrays(unsigned int id);             // Unload vertex data (VAO) from GPU memory
    LOADERTYPE void SFGLDeleteBuffers(unsigned int id);                  // Unload vertex data (VBO) from GPU memory
    LOADERTYPE void SFGLClearColor(byte r, byte g, byte b, byte a);      // Clear color buffer with color
    LOADERTYPE void SFGLClearScreenBuffers(void);                        // Clear used screen buffers (color and depth)
    LOADERTYPE void SFGLUpdateBuffer(int bufferId, void* data, int dataSize); // Update GPU buffer with new data
    LOADERTYPE unsigned int SFGLLoadAttribBuffer(unsigned int vaoId, int shadeSFGLoc, void* buffer, int size, bool dynamic);   // Load a new attributes buffer

    //------------------------------------------------------------------------------------
    // Functions Declaration - SFGL functionality
    //------------------------------------------------------------------------------------
    LOADERTYPE void SFGLInit(int width, int height);           // Initialize SFGL (buffers, shaders, textures, states)
    LOADERTYPE void SFGLClose(void);                           // De-inititialize SFGL (buffers, shaders, textures)
    LOADERTYPE void SFGLDraw(void);                            // Update and draw default internal buffers

    LOADERTYPE int SFGLGetVersion(void);                         // Returns current OpenGL version
    LOADERTYPE bool SFGLCheckBuffeSFGLimit(int vCount);            // Check internal buffer overflow for a given number of vertex
    LOADERTYPE void SFGLSetDebugMarker(const char* text);        // Set debug marker for analysis
    LOADERTYPE void SFGLLoadExtensions(void* loader);            // Load OpenGL extensions
    LOADERTYPE Vector3 SFGLUnproject(Vector3 source, Matrix proj, Matrix view);  // Get woSFGLd coordinates from screen coordinates

    // Textures data management
    LOADERTYPE unsigned int SFGLLoadTexture(void* data, int width, int height, int format, int mipmapCount); // Load texture in GPU
    LOADERTYPE unsigned int SFGLLoadTextureDepth(int width, int height, int bits, bool useRenderBuffer);     // Load depth texture/render buffer (to be attached to fbo)
    LOADERTYPE unsigned int SFGLLoadTextureCubemap(void* data, int size, int format);                        // Load texture cube map
    LOADERTYPE void SFGLUpdateTexture(unsigned int id, int width, int height, int format, const void* data); // Update GPU texture with new data
    LOADERTYPE void SFGLGetGlTextureFormats(int format, unsigned int* glInternalFormat, unsigned int* glFormat, unsigned int* glType);  // Get OpenGL internal formats
    LOADERTYPE void SFGLUnloadTexture(unsigned int id);                              // Unload texture from GPU memory

    LOADERTYPE void SFGLGenerateMipmaps(Texture2D* texture);                         // Generate mipmap data for selected texture
    LOADERTYPE void* SFGLReadTexturePixels(Texture2D texture);                       // Read texture pixel data
    LOADERTYPE unsigned char* SFGLReadScreenPixels(int width, int height);           // Read screen pixel data (color buffer)

    // Render texture management (fbo)
    LOADERTYPE RenderTexture2D SFGLLoadRenderTexture(int width, int height, int format, int depthBits, bool useDepthTexture);    // Load a render texture (with color and depth attachments)
    LOADERTYPE void SFGLRenderTextureAttach(RenderTexture target, unsigned int id, int attachType);  // Attach texture/render buffer to an fbo
    LOADERTYPE bool SFGLRenderTextureComplete(RenderTexture target);                 // Verify render texture is complete

    // Vertex data management
    LOADERTYPE void SFGLLoadMesh(Mesh* mesh, bool dynamic);                          // Upload vertex data into GPU and provided VAO/VBO ids
    LOADERTYPE void SFGLUpdateMesh(Mesh mesh, int buffer, int numVertex);            // Update vertex data on GPU (upload new data to one buffer)
    LOADERTYPE void SFGLDrawMesh(Mesh mesh, Material material, Matrix transform);    // Draw a 3d mesh with material and transform
    LOADERTYPE void SFGLUnloadMesh(Mesh* mesh);                                      // Unload mesh data from CPU and GPU

    // NOTE: There is a set of shader related functions that are available to end user,
    // to avoid creating function wrappers through core module, they have been directly declared in SFWGL.h
...
// Transform matrix to be used with SFGLTranslate, SFGLRotate, SFGLScale
...
//---------------------------------------
void SFGLMatrixMode(int mode)
{
    switch (mode)
    {
    case SFGL_PROJECTION: glMatrixMode(GL_PROJECTION); break;
    case SFGL_MODELVIEW: glMatrixMode(GL_MODELVIEW); break;
    case SFGL_TEXTURE: glMatrixMode(GL_TEXTURE); break;
    default: break;
    }
}

void SFGLFrustum(double left, double right, double bottom, double top, double znear, double zfar)
{
    glFrustum(left, right, bottom, top, znear, zfar);
}

void SFGLOrtho(double left, double right, double bottom, double top, double znear, double zfar)
{
    glOrtho(left, right, bottom, top, znear, zfar);
}

void SFGLPushMatrix(void) { glPushMatrix(); }
void SFGLPopMatrix(void) { glPopMatrix(); }
void SFGLLoadIdentity(void) { glLoadIdentity(); }
void SFGLTranslatef(float x, float y, float z) { glTranslatef(x, y, z); }
void SFGLRotatef(float angleDeg, float x, float y, float z) { glRotatef(angleDeg, x, y, z); }
void SFGLScalef(float x, float y, float z) { glScalef(x, y, z); }
void SFGLMultMatrixf(float* matf) { glMultMatrixf(matf); }
...
void SFGLMatrixMode(int mode)
{
    if (mode == SFGL_PROJECTION) currentMatrix = &projection;
    else if (mode == SFGL_MODELVIEW) currentMatrix = &modelview;
    //else if (mode == SFGL_TEXTURE) // Not supported

    currentMatrixMode = mode;
}

// Push the current matrix into stack
void SFGLPushMatrix(void)
{
    if (stackCounter >= MAX_MATRIX_STACK_SIZE) displayout(D_ERROR, "Matrix stack overflow");

    if (currentMatrixMode == SFGL_MODELVIEW)
    {
...
void SFGLPopMatrix(void)
{
    ...
    if ((stackCounter == 0) && (currentMatrixMode == SFGL_MODELVIEW))
    {
...
void SFGLLoadIdentity(void)
{
    *currentMatrix = MatrixIdentity();
}

// Multiply the current matrix by a translation matrix
void SFGLTranslatef(float x, float y, float z)
{
...
void SFGLRotatef(float angleDeg, float x, float y, float z)
{
...
void SFGLScalef(float x, float y, float z)
{
...
void SFGLMultMatrixf(float* matf)
{
...
void SFGLViewport(int x, int y, int width, int height)
{
...
// Fallback to OpenGL 1.1 function calls
//---------------------------------------
void SFGLBegin(int mode)
{
    switch (mode)
    {
    case SFGL_LINES: glBegin(GL_LINES); break;
    case SFGL_TRIANGLES: glBegin(GL_TRIANGLES); break;
    case SFGL_QUADS: glBegin(GL_QUADS); break;
    default: break;
    }
}

void SFGLEnd() { glEnd(); }
void SFGLVertex2i(int x, int y) { glVertex2i(x, y); }
void SFGLVertex2f(float x, float y) { glVertex2f(x, y); }
void SFGLVertex3f(float x, float y, float z) { glVertex3f(x, y, z); }
void SFGLTexCoord2f(float x, float y) { glTexCoord2f(x, y); }
void SFGLNormal3f(float x, float y, float z) { glNormal3f(x, y, z); }
void SFGLColor4ub(byte r, byte g, byte b, byte a) { glColor4ub(r, g, b, a); }
void SFGLColor3f(float x, float y, float z) { glColor3f(x, y, z); }
void SFGLColor4f(float x, float y, float z, float w) { glColor4f(x, y, z, w); }
...
void SFGLBegin(int mode)
{
    // Draw mode can be SFGL_LINES, SFGL_TRIANGLES and SFGL_QUADS
    ...
    if (draws[drawsCounter - 1].mode != mode)
    {
        if (draws[drawsCounter - 1].vertexCount > 0)
        {
            ...
            if (draws[drawsCounter - 1].mode == SFGL_LINES) draws[drawsCounter - 1].vertexAlignment = ((draws[drawsCounter - 1].vertexCount < 4) ? draws[drawsCounter - 1].vertexCount : draws[drawsCounter - 1].vertexCount % 4);
            else if (draws[drawsCounter - 1].mode == SFGL_TRIANGLES) draws[drawsCounter - 1].vertexAlignment = ((draws[drawsCounter - 1].vertexCount < 4) ? 1 : (4 - (draws[drawsCounter - 1].vertexCount % 4)));

            if (SFGLCheckBuffeSFGLimit(draws[drawsCounter - 1].vertexAlignment)) SFGLDraw();
            else
            {
        ...
        if (drawsCounter >= MAX_DRAWCALL_REGISTERED) SFGLDraw();
...
void SFGLEnd(void)
{
    ...
    // NOTE: Depth increment is dependant on SFGLOrtho(): z-near and z-far values,
    // as well as depth buffer bit-depth (16bit or 24bit or 32bit)
    // Correct increment formula would be: depthInc = (zfar - znear)/pow(2, bits)
    currentDepth += (1.0f / 20000.0f);

    // Verify internal buffers limits
    // NOTE: This check is combined with usage of SFGLCheckBuffeSFGLimit()
    if ((vertexData[currentBuffer].vCounter) >= (MAX_BATCH_ELEMENTS * 4 - 4))
    {
        // WARNING: If we are between SFGLPushMatrix() and SFGLPopMatrix() and we need to force a SFGLDraw(),
        // we need to call SFGLPopMatrix() before to recover *currentMatrix (modelview) for the next forced draw call!
...
void SFGLVertex3f(float x, float y, float z)
{
    Vector3 vec = { x, y, z };
    ...
    else displayout(D_ERROR, "MAX_BATCH_ELEMENTS overflow");
}

// Define one vertex (position)
void SFGLVertex2f(float x, float y)
{
    SFGLVertex3f(x, y, currentDepth);
}

// Define one vertex (position)
void SFGLVertex2i(int x, int y)
{
    SFGLVertex3f((float)x, (float)y, currentDepth);
}

// Define one vertex (texture coordinate)
// NOTE: Texture coordinates are limited to QUADS only
void SFGLTexCoord2f(float x, float y)
{
...
void SFGLNormal3f(float x, float y, float z)
{
    // TODO: Normals usage...
}

// Define one vertex (color)
void SFGLColor4ub(byte x, byte y, byte z, byte w)
{
...
// Define one vertex (color)
void SFGLColor4f(float r, float g, float b, float a)
{
    SFGLColor4ub((byte)(r * 255), (byte)(g * 255), (byte)(b * 255), (byte)(a * 255));
}

// Define one vertex (color)
void SFGLColor3f(float x, float y, float z)
{
    SFGLColor4ub((byte)(x * 255), (byte)(y * 255), (byte)(z * 255), 255);
}
...
void SFGLEnableTexture(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_11)
...
void SFGLDisableTexture(void)
{
    ...
    if (vertexData[currentBuffer].vCounter >= (MAX_BATCH_ELEMENTS * 4)) SFGLDraw();
#endif
}

// Set texture parameters (wrap mode/filter mode)
void SFGLTextureParameters(unsigned int id, int param, int value)
{
    glBindTexture(GL_TEXTURE_2D, id);

    switch (param)
    {
    case SFGL_TEXTURE_WRAP_S:
    case SFGL_TEXTURE_WRAP_T:
    {
        if (value == SFGL_WRAP_MIRROR_CLAMP)
        {
#if !defined(GRAPHICS_API_OPENGL_11)
            if (!texMirrorClampSupported) displayout(D_WARNING, "Clamp mirror wrap mode not supported");
#endif
        }
        else glTexParameteri(GL_TEXTURE_2D, param, value);
    } break;
    case SFGL_TEXTURE_MAG_FILTER:
    case SFGL_TEXTURE_MIN_FILTER: glTexParameteri(GL_TEXTURE_2D, param, value); break;
    case SFGL_TEXTURE_ANISOTROPIC_FILTER:
    {
#if !defined(GRAPHICS_API_OPENGL_11)
        ...
        {
            displayout(D_WARNING, "[TEX ID %i] Maximum anisotropic filter level supported is %iX", id, maxAnisotropicLevel);
            glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, (float)value);
        }
        else displayout(D_WARNING, "Anisotropic filtering not supported");
#endif
    } break;
    default: break;
    }
...
void SFGLEnableRenderTexture(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
void SFGLDisableRenderTexture(void)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
// Enable depth test
void SFGLEnableDepthTest(void)
{
    glEnable(GL_DEPTH_TEST);
}

// Disable depth test
void SFGLDisableDepthTest(void)
{
    glDisable(GL_DEPTH_TEST);
}

// Enable backface culling
void SFGLEnableBackfaceCulling(void)
{
    glEnable(GL_CULL_FACE);
}

// Disable backface culling
void SFGLDisableBackfaceCulling(void)
{
    glDisable(GL_CULL_FACE);
}

// Enable wire mode
void SFGLEnableWireMode(void)
{
#if defined (GRAPHICS_API_OPENGL_11) || defined(GRAPHICS_API_OPENGL_33)
...
void SFGLDeleteTextures(unsigned int id)
{
    if (id > 0) glDeleteTextures(1, &id);
}

// Unload render texture from GPU memory
void SFGLDeleteRenderTextures(RenderTexture2D target)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    ...
    displayout(D_INFO, "[FBO ID %i] Unloaded render texture data from VRAM (GPU)", target.id);
#endif
}

// Unload shader from GPU memory
void SFGLDeleteShader(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
void SFGLDeleteVertexArrays(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if (vaoSupported)
    {
        if (id != 0) glDeleteVertexArrays(1, &id);
        displayout(D_INFO, "[VAO ID %i] Unloaded model data from VRAM (GPU)", id);
    }
#endif
}

// Unload vertex data (VBO) from GPU memory
void SFGLDeleteBuffers(unsigned int id)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if (id != 0)
    {
        glDeleteBuffers(1, &id);
        if (!vaoSupported) displayout(D_INFO, "[VBO ID %i] Unloaded model vertex data from VRAM (GPU)", id);
    }
#endif
}

// Clear color buffer with color
void SFGLClearColor(byte r, byte g, byte b, byte a)
{
...
void SFGLClearScreenBuffers(void)
{
...
void SFGLUpdateBuffer(int bufferId, void* data, int dataSize)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
//----------------------------------------------------------------------------------
// Module Functions Definition - SFGL Functions
//----------------------------------------------------------------------------------

// Initialize SFGL: OpenGL extensions, default buffers/shaders/textures, OpenGL states
void SFGLInit(int width, int height)
{
    // Check OpenGL information and capabilities
    //------------------------------------------------------------------------------

    // Print current OpenGL and GLSL version
    displayout(D_INFO, "GPU: Vendor:   %s", glGetString(GL_VENDOR));
    displayout(D_INFO, "GPU: Renderer: %s", glGetString(GL_RENDERER));
    displayout(D_INFO, "GPU: Version:  %s", glGetString(GL_VERSION));
    displayout(D_INFO, "GPU: GLSL:     %s", glGetString(GL_SHADING_LANGUAGE_VERSION));

    // NOTE: We can get a bunch of extra information about GPU capabilities (glGet*)
    //int maxTexSize;
    //glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxTexSize);
    //displayout(D_INFO, "GL_MAX_TEXTURE_SIZE: %i", maxTexSize);
    ...
    //displayout(D_INFO, "GL_AUX_BUFFERS: %i", numAuxBuffers);

    //GLint numComp = 0;
    //GLint format[32] = { 0 };
    //glGetIntegerv(GL_NUM_COMPRESSED_TEXTURE_FORMATS, &numComp);
    //glGetIntegerv(GL_COMPRESSED_TEXTURE_FORMATS, format);
    //for (int i = 0; i < numComp; i++) displayout(D_INFO, "Supported compressed format: 0x%x", format[i]);

    // NOTE: We don't need that much data on screen... right now...

    // TODO: Automatize extensions loading using SFGLLoadExtensions() and GLAD
    // Actually, when SFGLInit() is called in InitWindow() in core.c,
    ...
    const char** extList = (const char**)SFGL_MALLOC(sizeof(const char*) * numExt);
    ...
    int len = stSFGLen(extensions) + 1;
    char* extensionsDup = (char*)SFGL_MALLOC(len);
    ...
    const char** extList = SFGL_MALLOC(sizeof(const char*) * 512);
    ...
    SFGL_FREE(extensionsDup);    // Duplicated string must be deallocated

    numExt -= 1;
#endif

    displayout(D_INFO, "Number of supported extensions: %i", numExt);

    // Show supported extensions
    //for (int i = 0; i < numExt; i++)  displayout(D_INFO, "Supported extension: %s", extList[i]);
...
#if defined(GRAPHICS_API_OPENGL_ES2)
    if (vaoSupported) displayout(D_INFO, "[EXTENSION] VAO extension detected, VAO functions initialized successfully");
    else displayout(D_WARNING, "[EXTENSION] VAO extension not found, VAO usage not supported");

    if (texNPOTSupported) displayout(D_INFO, "[EXTENSION] NPOT textures extension detected, full NPOT textures supported");
    else displayout(D_WARNING, "[EXTENSION] NPOT textures extension not found, limited NPOT support (no-mipmaps, no-repeat)");
#endif

    if (texCompDXTSupported) displayout(D_INFO, "[EXTENSION] DXT compressed textures supported");
    if (texCompETC1Supported) displayout(D_INFO, "[EXTENSION] ETC1 compressed textures supported");
    if (texCompETC2Supported) displayout(D_INFO, "[EXTENSION] ETC2/EAC compressed textures supported");
    if (texCompPVRTSupported) displayout(D_INFO, "[EXTENSION] PVRT compressed textures supported");
    if (texCompASTCSupported) displayout(D_INFO, "[EXTENSION] ASTC compressed textures supported");

    if (texAnisotropicFilterSupported) displayout(D_INFO, "[EXTENSION] Anisotropic textures filtering supported (max: %.0fX)", maxAnisotropicLevel);
    if (texMirrorClampSupported) displayout(D_INFO, "[EXTENSION] Mirror clamp wrap texture mode supported");

    if (debugMarkerSupported) displayout(D_INFO, "[EXTENSION] Debug Marker supported");

    // Initialize buffers, default shaders and default textures
    //----------------------------------------------------------
    // Init default white texture
    unsigned char pixels[4] = { 255, 255, 255, 255 };   // 1 pixel RGBA (4 bytes)
    defaultTextureId = SFGLLoadTexture(pixels, 1, 1, UNCOMPRESSED_R8G8B8A8, 1);

    if (defaultTextureId != 0) displayout(D_INFO, "[TEX ID %i] Base white texture loaded successfully", defaultTextureId);
    else displayout(D_WARNING, "Base white texture could not be loaded");
    ...
    draws = (DrawCall*)SFGL_MALLOC(sizeof(DrawCall) * MAX_DRAWCALL_REGISTERED);

    for (int i = 0; i < MAX_DRAWCALL_REGISTERED; i++)
    {
        draws[i].mode = SFGL_QUADS;
    ...
    displayout(D_INFO, "OpenGL default states initialized successfully");
}
...
void SFGLClose(void)
{
    ...
    displayout(D_INFO, "[TEX ID %i] Unloaded texture data (base white texture) from VRAM", defaultTextureId);
...
bool SFGLCheckBuffeSFGLimit(int vCount)
{
    bool overflow = false;
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
...
// Set debug marker
void SFGLSetDebugMarker(const char* text)
{
#if defined(GRAPHICS_API_OPENGL_33)
...
// Get woSFGLd coordinates from screen coordinates
Vector3 SFGLUnproject(Vector3 source, Matrix proj, Matrix view)
{
    ...
    // Normalized woSFGLd points in vectors
...
unsigned int SFGLLoadTexture(void* data, int width, int height, int format, int mipmapCount)
{
    ...
    {
        displayout(D_WARNING, "OpenGL 1.1 does not support GPU compressed texture formats");
        return id;
    }
#else
    ...
    {
        displayout(D_WARNING, "DXT compressed texture format not supported");
        return id;
    }
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((!texCompETC1Supported) && (format == COMPRESSED_ETC1_RGB))
    {
        displayout(D_WARNING, "ETC1 compressed texture format not supported");
        return id;
    }

    if ((!texCompETC2Supported) && ((format == COMPRESSED_ETC2_RGB) || (format == COMPRESSED_ETC2_EAC_RGBA)))
    {
        displayout(D_WARNING, "ETC2 compressed texture format not supported");
        return id;
    }

    if ((!texCompPVRTSupported) && ((format == COMPRESSED_PVRT_RGB) || (format == COMPRESSED_PVRT_RGBA)))
    {
        displayout(D_WARNING, "PVRT compressed texture format not supported");
        return id;
    }

    if ((!texCompASTCSupported) && ((format == COMPRESSED_ASTC_4x4_RGBA) || (format == COMPRESSED_ASTC_8x8_RGBA)))
    {
        displayout(D_WARNING, "ASTC compressed texture format not supported");
        return id;
    }
#endif
    ...
    displayout(D_DEBUG, "Load texture from data memory address: 0x%x", data);
    ...
    for (int i = 0; i < mipmapCount; i++)
    {
        ...
        SFGLGetGlTextureFormats(format, &glInternalFormat, &glFormat, &glType);

        displayout(D_DEBUG, "Load mipmap level %i (%i x %i), size: %i, offset: %i", i, mipWidth, mipHeight, mipSize, mipOffset);
    ...
    if (id > 0) displayout(D_INFO, "[TEX ID %i] Texture created successfully (%ix%i - %i mipmaps)", id, width, height, mipmapCount);
    else displayout(D_WARNING, "Texture could not be created");
...
unsigned int SFGLLoadTextureDepth(int width, int height, int bits, bool useRenderBuffer)
{
    unsigned int id = 0;
...
unsigned int SFGLLoadTextureCubemap(void* data, int size, int format)
{
...
void SFGLUpdateTexture(unsigned int id, int width, int height, int format, const void* data)
{
    glBindTexture(GL_TEXTURE_2D, id);
    ...
    else displayout(D_WARNING, "Texture format updating not supported");
}

// Get OpenGL internal formats and data type from SFWGL PixelFormat
void SFGLGetGlTextureFormats(int format, unsigned int* glInternalFormat, unsigned int* glFormat, unsigned int* glType)
{
    *glInternalFormat = -1;
    *glFormat = -1;
    *glType = -1;
...
#endif
    default: displayout(D_WARNING, "Texture format not supported"); break;
    }
}

//----------------------------------------------------------------------------------
// Module Functions Definition - Shaders Functions
// NOTE: Those functions are exposed directly to the user in SFWGL.h
//----------------------------------------------------------------------------------
...
char* LoadText(const char* fileName)
{
    ...
    if (fileName != NULL)
    {
        ...
        if (textFile != NULL)
        {
            ...
            if (size > 0)
            {
                text = (char*)SFGL_MALLOC(sizeof(char) * (size + 1));
        ...
        else displayout(D_WARNING, "[%s] Text file could not be opened", fileName);
    }
...
Shader LoadShaderCode(char* vsCode, char* fsCode)
{
    ...
    else
    {
        ...
        if (shader.id == 0)
        {
            displayout(D_WARNING, "Custom shader could not be loaded");
    ...
    for (int i = 0; i < uniformCount; i++)
    {
        ...
        displayout(D_DEBUG, "[SHDR ID %i] Active uniform [%s] set at location: %i", shader.id, name, location);
    }
#endif
...
Shader LoadShader(const char* vsFileName, const char* fsFileName)
{
    ...
    if (vShaderStr != NULL) SFGL_FREE(vShaderStr);
    if (fShaderStr != NULL) SFGL_FREE(fShaderStr);
...
void UnloadShader(Shader shader)
{
    if (shader.id > 0)
    {
        SFGLDeleteShader(shader.id);
        displayout(D_INFO, "[SHDR ID %i] Unloaded shader program data", shader.id);
    }
}
...
int GetShadeSFGLocation(Shader shader, const char* uniformName)
{
    int location = -1;
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    location = glGetUniformLocation(shader.id, uniformName);

    if (location == -1) displayout(D_WARNING, "[SHDR ID %i][%s] Shader uniform could not be found", shader.id, uniformName);
    else displayout(D_INFO, "[SHDR ID %i][%s] Shader uniform set at location: %i", shader.id, uniformName, location);
#endif
    return location;
}
...
void SetShaderValueV(Shader shader, int uniformLoc, const void* value, int uniformType, int count)
{
    ...
    default: displayout(D_WARNING, "Shader uniform could not be set data type not recognized");
    }
...
// Unload texture from GPU memory
void SFGLUnloadTexture(unsigned int id)
{
    if (id > 0) glDeleteTextures(1, &id);
}
...
RenderTexture2D SFGLLoadRenderTexture(int width, int height, int format, int depthBits, bool useDepthTexture)
{
    ...
    if ((format != -1) && (format < COMPRESSED_DXT1_RGB))
    {
        ...
        target.texture.id = SFGLLoadTexture(NULL, width, height, format, 1);
    ...
    if (depthBits > 0)
    {
        target.depth.id = SFGLLoadTextureDepth(width, height, depthBits, !useDepthTexture);
    ...
    //-----------------------------------------------------------------------------------------------------
    SFGLRenderTextureAttach(target, target.texture.id, 0);    // COLOR attachment
    SFGLRenderTextureAttach(target, target.depth.id, 1);      // DEPTH attachment
    //-----------------------------------------------------------------------------------------------------

    // Check if fbo is complete with attachments (valid)
    //-----------------------------------------------------------------------------------------------------
    if (SFGLRenderTextureComplete(target)) displayout(D_INFO, "[FBO ID %i] Framebuffer object created successfully", target.id);
    //-----------------------------------------------------------------------------------------------------
...
void SFGLRenderTextureAttach(RenderTexture2D target, unsigned int id, int attachType)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    glBindFramebuffer(GL_FRAMEBUFFER, target.id);
...
bool SFGLRenderTextureComplete(RenderTexture target)
{
    bool result = false;
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE)
    {
        switch (status)
        {
        case GL_FRAMEBUFFER_UNSUPPORTED: displayout(D_WARNING, "Framebuffer is unsupported"); break;
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT: displayout(D_WARNING, "Framebuffer has incomplete attachment"); break;
#if defined(GRAPHICS_API_OPENGL_ES2)
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS: displayout(D_WARNING, "Framebuffer has incomplete dimensions"); break;
#endif
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT: displayout(D_WARNING, "Framebuffer has a missing attachment"); break;
        default: break;
        }
    }
...
void SFGLGenerateMipmaps(Texture2D* texture)
{
    ...
    if (texIsPOT)
    {
        ...
        if (texture->format == UNCOMPRESSED_R8G8B8A8)
        {
            ...
            void* data = SFGLReadTexturePixels(*texture);
            ...
            SFGL_FREE(data); // Once mipmaps have been generated and data has been uploaded to GPU VRAM, we can discard RAM data

            displayout(D_WARNING, "[TEX ID %i] Mipmaps [%i] generated manually on CPU side", texture->id, texture->mipmaps);
        }
        else displayout(D_WARNING, "[TEX ID %i] Mipmaps could not be generated for texture format", texture->id);
    }
#elif defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if ((texIsPOT) || (texNPOTSupported))
    {
        ...
        displayout(D_INFO, "[TEX ID %i] Mipmaps generated automatically", texture->id);
...
#endif
    else displayout(D_WARNING, "[TEX ID %i] Mipmaps can not be generated", texture->id);

    glBindTexture(GL_TEXTURE_2D, 0);
}

// Upload vertex data into a VAO (if supported) and VBO
void SFGLLoadMesh(Mesh* mesh, bool dynamic)
{
    if (mesh->vaoId > 0)
    {
        // Check if mesh has already been loaded in GPU
        displayout(D_WARNING, "Trying to re-load an already loaded mesh");
        return;
    }
    ...
    if (vaoSupported)
    {
        if (mesh->vaoId > 0) displayout(D_INFO, "[VAO ID %i] Mesh uploaded successfully to VRAM (GPU)", mesh->vaoId);
        else displayout(D_WARNING, "Mesh could not be uploaded to VRAM (GPU)");
    }
    else
    {
        displayout(D_INFO, "[VBOs] Mesh uploaded successfully to VRAM (GPU)");
    }
#endif
}

// Load a new attributes buffer
unsigned int SFGLLoadAttribBuffer(unsigned int vaoId, int shadeSFGLoc, void* buffer, int size, bool dynamic)
{
    unsigned int id = 0;
    ...
    glVertexAttribPointer(shadeSFGLoc, 2, GL_FLOAT, 0, 0, 0);
    glEnableVertexAttribArray(shadeSFGLoc);
...
void SFGLUpdateMesh(Mesh mesh, int buffer, int numVertex)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    // Activate mesh VAO
...
void SFGLDrawMesh(Mesh mesh, Material material, Matrix transform)
{
#if defined(GRAPHICS_API_OPENGL_11)
    glEnable(GL_TEXTURE_2D);
    ...
    SFGLPushMatrix();
    SFGLMultMatrixf(MatrixToFloat(transform));
    SFGLColor4ub(material.maps[MAP_DIFFUSE].color.r, material.maps[MAP_DIFFUSE].color.g, material.maps[MAP_DIFFUSE].color.b, material.maps[MAP_DIFFUSE].color.a);
    ...
    // That's because BeginMode3D() sets it an no model-drawing function modifies it, all use SFGLPushMatrix() and SFGLPopMatrix()
...
void SFGLUnloadMesh(Mesh* mesh)
{
    SFGL_FREE(mesh->vertices);
    SFGL_FREE(mesh->texcoords);
    SFGL_FREE(mesh->normals);
    SFGL_FREE(mesh->colors);
    SFGL_FREE(mesh->tangents);
    SFGL_FREE(mesh->texcoords2);
    SFGL_FREE(mesh->indices);

    SFGL_FREE(mesh->animVertices);
    SFGL_FREE(mesh->animNormals);
    SFGL_FREE(mesh->boneWeights);
    SFGL_FREE(mesh->boneIds);

    SFGLDeleteBuffers(mesh->vboId[0]);   // vertex
    SFGLDeleteBuffers(mesh->vboId[1]);   // texcoords
    SFGLDeleteBuffers(mesh->vboId[2]);   // normals
    SFGLDeleteBuffers(mesh->vboId[3]);   // colors
    SFGLDeleteBuffers(mesh->vboId[4]);   // tangents
    SFGLDeleteBuffers(mesh->vboId[5]);   // texcoords2
    SFGLDeleteBuffers(mesh->vboId[6]);   // indices

    SFGLDeleteVertexArrays(mesh->vaoId);
}

// Read screen pixel data (color buffer)
unsigned char* SFGLReadScreenPixels(int width, int height)
{
    unsigned char* screenData = (unsigned char*)SFGL_CALLOC(width * height * 4, sizeof(unsigned char));

    // NOTE 1: glReadPixels returns image flipped vertically -> (0,0) is the bottom left corner of the framebuffer
    // NOTE 2: We are getting alpha channel! Be careful, it can be transparent if not cleared propeSFGLy!
    glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE, screenData);

    // Flip image vertically!
    unsigned char* imgData = (unsigned char*)SFGL_MALLOC(width * height * sizeof(unsigned char) * 4);
    ...
    SFGL_FREE(screenData);

    return imgData;     // NOTE: image data should be freed
}

// Read texture pixel data
void* SFGLReadTexturePixels(Texture2D texture)
{
    void* pixels = NULL;
    ...
    unsigned int glInternalFormat, glFormat, glType;
    SFGLGetGlTextureFormats(texture.format, &glInternalFormat, &glFormat, &glType);
    ...
    {
        pixels = (unsigned char*)SFGL_MALLOC(size);
        glGetTexImage(GL_TEXTURE_2D, 0, glFormat, glType, pixels);
    }
    else displayout(D_WARNING, "Texture data retrieval not suported for pixel format");
    ...
    RenderTexture2D fbo = SFGLLoadRenderTexture(texture.width, texture.height, UNCOMPRESSED_R8G8B8A8, 16, false);
    ...
    pixels = (unsigned char*)SFGL_MALLOC(GetPixelDataSize(texture.width, texture.height, texture.format));
...
Texture2D GenTexturePrefilter(Shader shader, Texture2D cubemap, int size)
{
    ...
    int roughnessLoc = GetShadeSFGLocation(shader, "roughness");
...
{
    SFGLDraw();             // Force drawing elements
...
void InitVrSimulator(void)
{
    ...
    stereoFbo = SFGLLoadRenderTexture(screenWidth, screenHeight, UNCOMPRESSED_R8G8B8A8, 24, false);

    vrSimulatorReady = true;
#else
    displayout(D_WARNING, "VR Simulator not supported on OpenGL 1.1");
#endif
}
...
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
    if (vrSimulatorReady) SFGLDeleteRenderTextures(stereoFbo);        // Unload stereo framebuffer and texture
#endif
}
...
void SetVrConfiguration(VrDeviceInfo hmd, Shader distortion)
{
    ...
    displayout(D_DEBUG, "VR: Distortion Scale: %f", distortionScale);
    ...
    displayout(D_DEBUG, "VR: Distortion Shader: LeftLensCenter = { %f, %f }", leftLensCenter[0], leftLensCenter[1]);
    displayout(D_DEBUG, "VR: Distortion Shader: RightLensCenter = { %f, %f }", rightLensCenter[0], rightLensCenter[1]);
    displayout(D_DEBUG, "VR: Distortion Shader: Scale = { %f, %f }", scale[0], scale[1]);
    displayout(D_DEBUG, "VR: Distortion Shader: ScaleIn = { %f, %f }", scaleIn[0], scaleIn[1]);
    ...
    if (vrConfig.distortionShader.id > 0)
    {
        ...
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "leftLensCenter"), leftLensCenter, UNIFORM_VEC2);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "rightLensCenter"), rightLensCenter, UNIFORM_VEC2);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "leftScreenCenter"), leftScreenCenter, UNIFORM_VEC2);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "rightScreenCenter"), rightScreenCenter, UNIFORM_VEC2);

        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "scale"), scale, UNIFORM_VEC2);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "scaleIn"), scaleIn, UNIFORM_VEC2);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "hmdWarpParam"), hmd.lensDistortionValues, UNIFORM_VEC4);
        SetShaderValue(vrConfig.distortionShader, GetShadeSFGLocation(vrConfig.distortionShader, "chromaAbParam"), hmd.chromaAbCorrection, UNIFORM_VEC4);
    }
#endif
}
...
void ToggleVrMode(void)
{
    ...
    if (!vrSimulatorReady)
    {
        ...
        SFGLViewport(0, 0, screenWidth, screenHeight);
...
void BeginVrDrawing(void)
{
    ...
    if (vrSimulatorReady)
    {
        ...
        SFGLEnableRenderTexture(stereoFbo.id);    // Setup framebuffer for stereo rendering
        //glEnable(GL_FRAMEBUFFER_SRGB);        // Enable SRGB framebuffer (only if required)

        //glViewport(0, 0, buffer.width, buffer.height);    // Useful if rendering to separate framebuffers (every eye)
        SFGLClearScreenBuffers();                 // Clear current framebuffer
...
void EndVrDrawing(void)
{
    ...
    if (vrSimulatorReady)
    {
        ...
        SFGLDisableRenderTexture();       // Unbind current framebuffer
        ...
        // Let SFGL reconfigure internal matrices
        SFGLMatrixMode(SFGL_PROJECTION);                            // Enable internal projection matrix
        SFGLLoadIdentity();                                       // Reset internal projection matrix
        SFGLOrtho(0.0, screenWidth, screenHeight, 0.0, 0.0, 1.0); // Recalculate internal projection matrix
        SFGLMatrixMode(SFGL_MODELVIEW);                             // Enable internal modelview matrix
        SFGLLoadIdentity();                                       // Reset internal modelview matrix
        ...
        SFGLEnableTexture(stereoFbo.texture.id);

        SFGLPushMatrix();
        SFGLBegin(SFGL_QUADS);
        SFGLColor4ub(255, 255, 255, 255);
        SFGLNormal3f(0.0f, 0.0f, 1.0f);

        // Bottom-left corner for texture and quad
        SFGLTexCoord2f(0.0f, 1.0f);
        SFGLVertex2f(0.0f, 0.0f);

        // Bottom-right corner for texture and quad
        SFGLTexCoord2f(0.0f, 0.0f);
        SFGLVertex2f(0.0f, (float)stereoFbo.texture.height);

        // Top-right corner for texture and quad
        SFGLTexCoord2f(1.0f, 0.0f);
        SFGLVertex2f((float)stereoFbo.texture.width, (float)stereoFbo.texture.height);

        // Top-left corner for texture and quad
        SFGLTexCoord2f(1.0f, 1.0f);
        SFGLVertex2f((float)stereoFbo.texture.width, 0.0f);
        SFGLEnd();
        SFGLPopMatrix();
...
static unsigned int CompileShader(const char* shaderStr, int type)
{
    ...
    if (success != GL_TRUE)
    {
        displayout(D_WARNING, "[SHDR ID %i] Failed to compile shader...", shader);
        int maxLength = 0;
        int length;
    ...
    else displayout(D_INFO, "[SHDR ID %i] Shader compiled successfully", shader);
...
static unsigned int LoadShaderProgram(unsigned int vShaderId, unsigned int fShaderId)
{
    ...
    if (success == GL_FALSE)
    {
        displayout(D_WARNING, "[SHDR ID %i] Failed to link shader program...", program);
    ...
    else displayout(D_INFO, "[SHDR ID %i] Shader program loaded successfully", program);
#endif
    return program;
}
...
static Shader LoadShaderDefault(void)
{
    ...
    if (shader.id > 0)
    {
        displayout(D_INFO, "[SHDR ID %i] Default shader loaded successfully", shader.id);
    ...
    else displayout(D_WARNING, "[SHDR ID %i] Default shader could not be loaded", shader.id);
...
static void LoadBuffersDefault(void)
{
    ...
    {
        vertexData[i].vertices = (float*)SFGL_MALLOC(sizeof(float) * 3 * 4 * MAX_BATCH_ELEMENTS);        // 3 float by vertex, 4 vertex by quad
        vertexData[i].texcoords = (float*)SFGL_MALLOC(sizeof(float) * 2 * 4 * MAX_BATCH_ELEMENTS);       // 2 float by texcoord, 4 texcoord by quad
        vertexData[i].colors = (unsigned char*)SFGL_MALLOC(sizeof(unsigned char) * 4 * 4 * MAX_BATCH_ELEMENTS);  // 4 float by color, 4 colors by quad
#if defined(GRAPHICS_API_OPENGL_33)
        vertexData[i].indices = (unsigned int*)SFGL_MALLOC(sizeof(unsigned int) * 6 * MAX_BATCH_ELEMENTS);      // 6 int by quad (indices)
#elif defined(GRAPHICS_API_OPENGL_ES2)
        vertexData[i].indices = (unsigned short*)SFGL_MALLOC(sizeof(unsigned short) * 6 * MAX_BATCH_ELEMENTS);  // 6 int by quad (indices)
#endif
    ...
    displayout(D_INFO, "Internal buffers initialized successfully (CPU)");
    //--------------------------------------------------------------------------------------------
    ...
    displayout(D_INFO, "Internal buffers uploaded successfully (GPU)");
    ...
    for (int eye = 0; eye < eyesCount; eye++)
    {
        ...
        if (vertexData[currentBuffer].vCounter > 0)
        {
            ...
            for (int i = 0; i < drawsCounter; i++)
            {
                ...
                if ((draws[i].mode == SFGL_LINES) || (draws[i].mode == SFGL_TRIANGLES)) glDrawArrays(draws[i].mode, vertexOffset, draws[i].vertexCount);
                else
                {
#if defined(GRAPHICS_API_OPENGL_33)
...
static void UnloadBuffersDefault(void)
{
    ...
    for (int i = 0; i < MAX_BATCH_BUFFERING; i++)
    {
        ...
        SFGL_FREE(vertexData[i].vertices);
        SFGL_FREE(vertexData[i].texcoords);
        SFGL_FREE(vertexData[i].colors);
        SFGL_FREE(vertexData[i].indices);
    }
}
